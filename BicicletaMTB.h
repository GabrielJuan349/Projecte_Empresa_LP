...
public:
	...
	BicicletaMTB();
	BicicletaMTB(const string& model, string descripcio, const int& temporada, const Talla& talla,
		const Quadre& quadre, const Roda& roda, const Fre& fre, TipusBicicleta tipus, Categoria categoria,
		Modalitat modalitat, bool electrica) : Bicicleta(model, descripcio, temporada, talla, quadre, Roda::RODA_27, Fre::DISC, TipusBicicleta::MTB),
		m_categoria(categoria), m_modalitat(modalitat), m_electrica(electrica) {}
